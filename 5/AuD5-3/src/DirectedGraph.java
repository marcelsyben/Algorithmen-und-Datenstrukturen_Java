...
class DirectedGraph {
    ...
    public void BFSrec(Node anfang, Node ziel, int max, Queue<Node> queue) {
        for (int i = 0; i < anfang.neighbors.size(); i++) {
            if (anfang.neighbors.get(i).dest == ziel) {
                return;
            }
            else {
            queue.add(anfang.neighbors.get(i).dest);
            }
        }

        for (int i = 0; i < anfang.neighbors.size(); i++) {

            BFSrec(queue., ziel, max, queue);
        }
